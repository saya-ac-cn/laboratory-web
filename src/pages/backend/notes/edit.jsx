...
import {Button, Form, Col, Input, Tag, Icon, Select} from "antd";
...
class EditPage extends Component {
    ...
    componentWillMount() {
        ...
        _this.getNoteBooks()
        // 取出携带的state
        let notes = _this.props.location.state;  // 如果是添加没值, 否则有值
        if (notes){
                ...
                <section className="b-edit-notes-page">
                    <div className="mytips">
                        ...
                        <blockquote>
                            ...
                            <p>
                                <i>4、动态内容必须填写，书写采用暂采用Html语法，后期将升级为MarkDown语法。书写完毕后，建议到前台页面查看实际效果，以便造成格式不兼容。</i>
                            </p>
                        </blockquote>
                    </div>
                    <Col span={24} className="b-edit-notes-page-form">
                        <Form {...this.formItemLayout} className="bk-transparent">
                            ...
                            <Form.Item label="所属分类" {...this.formItemLayout}>
                                {
                                    getFieldDecorator('notebookId', {
                                        initialValue: form.notebookId,
                                        rules: [
